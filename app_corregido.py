...
for grupo in grupos:
    ...
    for i, cat in enumerate(cat_grupo_existentes):
        ...
        # Máximo valor entre todos los jugadores de la jornada para esa posición y categoría
        max_valor_jornada = p90ap24[p90ap24["POSICION"] == posicion][cat].values[0]


        # Normalizar a porcentaje
        val_cl25_pct = (valor_cl25 / max_valor_jornada) * 100 if max_valor_jornada != 0 else 0
        val_ap24_pct = (valor_ap24 / max_valor_jornada) * 100 if max_valor_jornada != 0 else 0

        fig.add_trace(go.Bar(
            y=[cat], x=[-val_cl25_pct], orientation='h',
            marker_color=obtener_color(val_cl25_pct),
            text=[f"{val_cl25_pct:.0f}%"],  # ← Mostrar porcentaje
            textposition="outside", showlegend=(i == 0), width=0.35, name="CL25"
        ))

        fig.add_trace(go.Bar(
            y=[cat], x=[val_ap24_pct], orientation='h',
            marker_color=obtener_color(val_ap24_pct),
            text=[f"{val_ap24_pct:.0f}%"],  # ← Mostrar porcentaje
            textposition="outside", showlegend=(i == 0), width=0.35, name="AP24"
        ))
